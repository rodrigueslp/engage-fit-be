...
func (g SafeGateway) Send(ctx context.Context, settings domain.WhatsappSettings, message services.WhatsappMessage) (*services.WhatsappSendResult, error) {
	...
	if len(g.devAllowedRecipientPhones) > 0 {
		if _, ok := g.devAllowedRecipientPhones[normalizePhone(message.Phone)]; !ok {
			return nil, fmt.Errorf("recipient %s is not allowed for real WhatsApp send in %s", message.Phone, g.appEnv)
		}
		return g.next.Send(ctx, settings, message)
	}

	if g.devRecipientPhone == "" {
		return nil, fmt.Errorf("WHATSAPP_DEV_ALLOWED_RECIPIENT_PHONES or WHATSAPP_DEV_RECIPIENT_PHONE is required for real WhatsApp send in %s", g.appEnv)
	}
